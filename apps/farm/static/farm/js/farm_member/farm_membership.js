...
function loadMembershipList(farmId, currentUsername, currentUserRole, fetchedMemberships, $container, $tbody) {
...
function showMembershipForm(farmId, membershipId, membershipModal) {
    if (!isValidFarmId(farmId)) {
...
function bindEvents(membershipModal, fetchedMemberships, lastFetchedUser) {
    ...
    // Submit form thêm/sửa thành viên
    $(document).on('submit', '#membership-form', function (e) {
        ...
        loadMembershipList(farmId, lastFetchedUser.username, lastFetchedUser.role, fetchedMemberships, $('.membership-container[data-farm-id="' + farmId + '"]'), $(`.membership-container[data-farm-id="${farmId}"] tbody`));
...
export async function initMemberships() {
  ...
  const lastFetchedUser = { username: '', role: '' };
  ...
  fetchCurrentUser((user) => {
    ...
    loadMembershipList(Number(farmIdRaw), lastFetchedUser.username, lastFetchedUser.role, fetchedMemberships, $container, $container.find('tbody'));
  }, () => {
