...
function loadDocumentList(farmId) {
    ...
    fetchFarmDocuments(farmId, (docs) => {
        console.log(docs);
...
function showDocumentForm(farmId, documentId = null) {
  ...
  if (documentId) {
    ...
    fetchFarmDocumentDetail(documentId, (doc) => {
      $('#title').val(doc.title);
...
function bindDocumentEvents() {
  ...
  $(document).on('submit', '#document-form', function (e) {
    ...
    const docId = $(this).attr('data-id') || null;
    const formData = new FormData(this);

    createOrUpdateFarmDocument(farmId, docId, formData, () => {
      documentModal.hide();
...
export async function initDocuments() {
  ...
  if (isValidFarmId(farmIdFromLocal)) {
    ...
    loadDocumentList(farmIdFromLocal);
  }
}
